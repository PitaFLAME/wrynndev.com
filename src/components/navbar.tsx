...
const Navbar = ({ isNavTriggered, activePage } : {
  ...
  return (
    <>
      <div
        ref={navRef}
        className={`fixed flex justify-center w-full md:h-24 h-16 md:py-4 py-2 px-8 z-40 transition-all duration-75 
      ...
      >
        <nav className="flex flex-row w-full h-full max-w-[1620px] px-[2vw]">
          ...
          <button className="md:hidden flex ml-auto my-auto" onClick={() => setIsMenuActive(true)}>
      ...
      {/* Side Menu */}
      <div className={`${isMenuActive ? 'fixed' : 'hidden'} h-screen w-5/6 bg-purple-500 z-50`}>
